...
def test_create_input_phrases(con: DuckDBPyConnection) -> None:
    ...
    expected = {
        (1, "VIC 3056"), (1, "SYDNEY RD"), (1, "RD BRUNSWICK"),
        (1, "BRUNSWICK VIC"), (1, "115 SYDNEY"), (1, "115 RD"),
        (2, "504 SYDNEY"), (2, "504 RD"), (2, "SYDNEY RD"), (2, "RD BRUNSWICK"),
        (3, "62 DAWSON"), (3, "62 ST"), (3, "DAWSON ST"),
        (3, "ST BRUNSWICK"), (3, "BRUNSWICK EAST"),
    }
    ...
    expected = pd.DataFrame(
        {
            ...
            "latitude": np.array([-37.774810791015625, -37.77077102661133], dtype='float32'),
            "longitude": np.array([144.9606170654297, 144.95619201660156], dtype='float32'),
        }
    )
    pd.testing.assert_frame_equal(result, expected)
    ...
    expected = pd.DataFrame(
        {
            'address_id1': np.array([1, 3], dtype='int32'), 
            'address_id2': np.array([7174666, 12721438], dtype='int32'),
            'address': ['115 SYDNEY RD BRUNSWICK VIC 3056', '62 DAWSON ST BRUNSWICK EAST'], 
            'input_numerics': [
                np.array(['115', '3056'], dtype=object),
                np.array(['62'], dtype=object),
            ],
...
def test_neighbouring_suburb_match(con: DuckDBPyConnection) -> None:
    ...
    assert result.neighbouring_suburb_correction.value_counts().to_dict() == {True: 16, False: 2}
    assert result.address.value_counts().to_dict() == {'115 SYDNEY RD BRUNSWICK VIC 3056': 9, '62 DAWSON ST BRUNSWICK EAST': 9}
...
def test_rank_by_similarity(con: DuckDBPyConnection) -> None:
    ...
    assert result['rank'].value_counts().to_dict() == {1: 2, 8: 2, 7: 2, 6: 2, 9: 2, 4: 2, 3: 2, 2: 2, 5: 2}
    ...
    expected = pd.DataFrame(
        {
            ...
            'input_address': np.array(['115 SYDNEY RD BRUNSWICK VIC 3056', '62 DAWSON ST BRUNSWICK EAST'], dtype='object'),
            'address_matched': ['115 SYDNEY RD BRUNSWICK VIC 3056', '62 DAWSON ST BRUNSWICK VIC 3056'], 
            'suburb': ['BRUNSWICK', 'BRUNSWICK'], 
            'postcode': np.array([3056, 3056], dtype='int32'), 
            'latitude': np.array([-37.774810791015625, -37.77077102661133], dtype='float32'), 
            'longitude': np.array([144.9606170654297, 144.95619201660156], dtype='float32'), 
            'similarity': np.array([1.0, 0.375], dtype='float64'), 
            'match_numerics': [np.array(['115', '3056'], dtype=object), np.array(['62', '3056'], dtype=object)], 
            'match_alpha_tokens': [np.array(['SYDNEY', 'RD', 'BRUNSWICK', 'VIC'], dtype=object), np.array(['DAWSON', 'ST', 'BRUNSWICK', 'VIC'], dtype=object)],
            'input_numerics': [np.array(['115', '3056'], dtype=object), np.array(['62'], dtype=object)], 
            'input_alpha_tokens': [np.array(['SYDNEY', 'RD', 'BRUNSWICK', 'VIC'], dtype=object), np.array(['DAWSON', 'ST', 'BRUNSWICK', 'EAST'], dtype=object)], 
    ...
    expected = pd.DataFrame(
        {
            ...
            'input_address': np.array(
                ['115 SYDNEY RD BRUNSWICK VIC 3056', '504 SYDNEY RD BRUNSWICK', '62 DAWSON ST BRUNSWICK EAST'],
                dtype=object,
            ),
            'address_matched': np.array(
                ['115 SYDNEY RD BRUNSWICK VIC 3056', np.nan, '62 DAWSON ST BRUNSWICK VIC 3056'],
                dtype=object,
            ),
            'suburb': np.array(['BRUNSWICK', np.nan, 'BRUNSWICK'], dtype=object),
            'postcode': pd.array([3056, pd.NA, 3056], dtype='Int32'),
            'latitude': np.array([-37.774810791015625, np.nan, -37.77077102661133], dtype='float32'),
            'longitude': np.array([144.9606170654297, np.nan, 144.95619201660156], dtype='float32'),
            ...
            'neighbouring_suburb_correction': pd.Series([False, pd.NA, False], dtype='boolean'),
        }
    )
