...
select t1.addr_id, soundex(t1.token) || ' ' || soundex(t2.token) tokenphrase
from tokens t1
left join tokens t2
...
from tokens t1
left join tokens t2
on (t1.addr_id, t1.row_num)=(t2.addr_id, t2.row_num-2) -- for next to nearest neighbour phrases that start with numeric token
where tokenphrase is not null
