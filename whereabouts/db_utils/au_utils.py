...
def load_state_data(db, folder, state_name):
    ...
    filenames_state = [
        filename for filename in os.listdir(folder) if state_name in filename
    ]
    ...
    for filename in filenames_state:
        ...
        db.execute(
            f"insert into {table_name} select * from read_csv('{filename}', delim='|')"
        )
...
def process_gnaf_core(filename_in, filename_out):
    ...
    try:
        ...
        if gnaf_schema_valid(df.columns, columns):
            ...
            df2 = df2.with_columns(row_num=np.arange(df2.shape[0]))
