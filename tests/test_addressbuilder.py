...
def test_table_creation() -> None:
    db_name: str = "db_test.db"
    table_names: npt.NDArray[np.object_] = np.array(
        [
            "addrtext",
            "numbers",
            "phrase",
    ...
    assert (
        addressloader.con.execute("show tables;").df().name.values == table_names
    ).all()
...
def test_create_final_address_table() -> None:
    db_name: str = "db_test.db"
    colnames_full_addresses: npt.NDArray[np.object_] = np.array(
        [
            "addr_id",
            "addr",
            "numeric_tokens",
    ...
    assert (
        addressloader.con.execute("select * from addrtext_with_detail").df().shape[0]
        == 18687
    )
    assert (
        addressloader.con.execute("select * from addrtext_with_detail").df().shape[1]
        == 9
    )
    assert (
        addressloader.con.execute("select * from addrtext_with_detail").df().columns
        == colnames_full_addresses
    ).all()
...
def test_create_standard_phrases() -> None:
    ...
    assert (
        addressloader.con.execute("select count(*) from phrase").df().values[0, 0]
        == 143076
    )
...
def test_create_inverted_index() -> None:
    ...
    assert (
        addressloader.con.execute("select count(*) from phraseinverted")
        .df()
        .values[0, 0]
        == 21927
    )
